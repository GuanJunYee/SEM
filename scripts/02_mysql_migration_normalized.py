...
LOG = RESULTS / "02_mysql_normalized_load_log.txt"
DDL = RESULTS / "02_mysql_normalized_schema.sql"
...
def load_transactions_table(engine):
    ...
    with engine.connect() as conn:
        ...
        if reconcile_batch_data(engine, "Transactions", expected_total, " (final check)"):
            log("[OK] All transaction data successfully migrated and verified")
        else:
...
def reconcile_batch_data(engine, table_name, expected_count, batch_info=""):
    ...
    with engine.connect() as conn:
        ...
        if actual_count >= expected_count:
            log(f"[OK] Batch reconciliation passed for {table_name}{batch_info}: Expected {expected_count}, Got {actual_count}")
...
def audit_log(event_type, message, extra_data=None):
    ...
    # Log to audit file
    audit_file = RESULTS / "02_migration_audit.log"
...
def main():
    """Main execution function with audit logging"""
