"""
03_migrate_to_mysql_normalized.py
...
# Input files (created by 02_clean_dataset_normalized.py)
CATEGORIES_CSV = DATA / "categories.csv"
LOCATIONS_CSV = DATA / "locations.csv"
PAYMENT_METHODS_CSV = DATA / "payment_methods.csv"
CUSTOMERS_CSV = DATA / "customers.csv"
ITEMS_CSV = DATA / "items.csv"
TRANSACTIONS_CSV = DATA / "transactions_normalized.csv"
...
def load_reference_tables(engine):
    ...
    if CATEGORIES_CSV.exists():
        ...
        log(f"Loading {len(categories_df)} categories...")
        
        # Insert categories one by one to get proper auto-increment IDs
        with engine.begin() as conn:
            for _, row in categories_df.iterrows():
                conn.execute(text(
                    "INSERT IGNORE INTO `Categories` (`CategoryName`) VALUES (:name)"
    ...
    if LOCATIONS_CSV.exists():
        ...
        log(f"Loading {len(locations_df)} locations...")
        
        with engine.begin() as conn:
            for _, row in locations_df.iterrows():
                conn.execute(text(
                    "INSERT IGNORE INTO `Locations` (`LocationName`) VALUES (:name)"
    ...
    if PAYMENT_METHODS_CSV.exists():
        ...
        log(f"Loading {len(payment_methods_df)} payment methods...")
        
        with engine.begin() as conn:
            for _, row in payment_methods_df.iterrows():
                conn.execute(text(
                    "INSERT IGNORE INTO `PaymentMethods` (`PaymentMethodName`) VALUES (:name)"
    ...
    if CUSTOMERS_CSV.exists():
        ...
        log(f"Loading {len(customers_df)} customers...")
        
        with engine.begin() as conn:
            for _, row in customers_df.iterrows():
                conn.execute(text(
                    "INSERT IGNORE INTO `Customers` (`CustomerID`) VALUES (:id)"
                ), {"id": row['CustomerID']})
...
def load_items_table(engine):
    """Load items table (depends on Categories)"""
    ...
    # Load items with proper CategoryID references
    with engine.begin() as conn:
        for _, row in items_df.iterrows():
            ...
            conn.execute(text("""
                INSERT IGNORE INTO `Items` (`ItemName`, `PricePerUnit`, `CategoryID`) 
...
def main():
    ...
    try:
        ...
        if missing_files:
            log("ERROR: Missing required CSV files:")
